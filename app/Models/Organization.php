...
class Organization extends Model
{
    ...
    public static function filter_search(){
        ...
        if ( $type !== null ) {
            ...
            if( $type === 'Has Users' ) {
                ...
                $query->whereIn('id', function ($query) {
                    $query->select('organization_id')->from('persons');
                });
            }
        ...
        if ($order !== null) {
            ...
            if( $order === 'A - Z' ) {
                ...
                $query->orderBy('organizations.name', 'DESC');

            }elseif( $order === 'Recent added' ) {

                $query->latest('organizations.created_at');
            }
        ...
        foreach($searched_data as $d) {
            ...
            $d->count =Person::leftjoin('organizations as org', 'persons.organization_id', '=', 'org.id')
            ->where('org.id',$d->id)
            ->count();
    ...
    public static function addNew($name, $rtn = 'object') {
        $obj = Organization::firstOrNew(['name' =>  $name]);
        $obj->name = $name;
        $obj->deleted_at = null;
