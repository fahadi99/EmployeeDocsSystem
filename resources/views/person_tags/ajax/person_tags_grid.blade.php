    ...
    <div class="card card-custom pl-2 pt-5 pb-5" id="kt_card_1">
        <div class="card-header border-0 pr-3">
            <div class="card-title w-70 max-w-70" style="word-break : break-all">
                <h3 class="card-label">{{$row['tag_name']}}

                </h3>
            </div>
            <div class="card-toolbar">
                @if(checkPersonPermission('update-person-tags-3-0'))
                    <a  data-id="{{$row['id']}}"  href="#" class="person-tags-edit btn btn-icon btn-sm btn-hover-light-primary"  data-toggle="modal" data-placement="top" title="Edit Group(COO)"  data-target="#exampleModal2">
                        <i class="fas fa-pencil-alt"></i>
                    </a>
                @endif
                @if(checkPersonPermission('delete-person-tags-3-0'))
                    <a class="person-tags-delete" data-id="{{$row['id']}}" href="#" class="btn btn-icon btn-sm btn-hover-light-primary mr-1" data-toggle="modal"   data-placement="top" title="Delete Group(COO) " data-target="#exampleModal3">
